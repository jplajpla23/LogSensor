            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="form" >
                        ...
                        <form style="margin: 25px!important;" method="POST" action="{{route('editAlertSave')}}">
                            @csrf
                            <input type="hidden" name="id" value="{{ $a->id}}">
                            <div class="row">
                                <div class="form-group" style="width: 98%!important;" >
                                    <label for="desc">Description</label>
                                    <input type="text" class="form-control col-md-12" id="name" name="desc" required value="{{ $a->Description}}">
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group" style="width: 98%!important;" >
                                    <label for="message">Message To Send</label>
                                    <textarea name="message" class="form-control" required>{{$a->Message}} </textarea>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group"  style="width: 98%!important;">
                                    <label for="min">Min of Safe Range</label>
                                    <input type="number" class="form-control col-md-12" id="min" step="0.01"name="min" onchange="minchange(this);" required value="{{$a->min}}">
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group" style="width: 98%!important;" >
                                    <label for="max">Max of Safe Range</label>
                                    <input type="number" class="form-control col-md-12" id="max" step="0.01" name="max" onchange="maxinchange(this);" value="{{ $a->max }}" required>
                                </div>
                            </div>
